...
- **`scorecard.yml` keeps its ref-only key, as a recorded exception rather than an
  oversight.** #100 changed it along with `ci.yml` and should not have. CI-CD-STANDARD.md
  11c allows a ref-only key exactly where the newest run is the only one that matters: an
  OpenSSF score is a property of the repository, not of a commit, so a superseded run was
  not a different answer, and giving each commit its own scan spends runner minutes to
  preserve a verdict nobody reads. The standard's test for the exception is that the
  workflow gates no merge, and
  ...
  exemption stops being safe and the build says so. An exception list nobody re-checks is
  how a real offender keeps its pass.
