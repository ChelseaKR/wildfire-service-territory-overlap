...
league table of which fires happened where, labeled with company names.
...
src/wildfire_service_territory_overlap/
  ...
  page.py        the same document, served: site/index.html, gated in two engines
  cli.py         the build: files on disk in, both artifacts out
published/       measurements.json and REPORT.md, from the real retrievals
site/            index.html, a rendering of published/measurements.json
...
license and by the California Energy Commission under its own conditions of use, and is
